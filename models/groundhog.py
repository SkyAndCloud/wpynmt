...
from models.backward_decoder import BackwardDecoder
...
class Decoder(nn.Module):
    ...
    def __init__(self, trg_vocab_size, max_out=True, classifier=None, with_ln=False):
        ...
        self.assist_attention = Attention(wargs.dec_hid_size, wargs.align_size, scale=True)
        ...
        self.tanh = nn.Tanh()
        self.gru = GRU(wargs.trg_wemb_size, wargs.dec_hid_size, enc_hid_size=2*wargs.enc_hid_size+wargs.dec_hid_size, with_ln=with_ln)
        ...
        self.lc = nn.Linear(2 * wargs.enc_hid_size + wargs.dec_hid_size, out_size)
    ...
    def forward(self, s_tm1, xs_h, ys, uh, xs_mask=None, ys_mask=None, assist_states=None, assist_states_m=None):
        ...
        for k in range(y_Lm1):
            ...
            if isinstance(assist_states, Variable):
                context, s_tm1, _, assist_alpha = self.step(s_tm1, xs_h, uh, y_tm1,
                                                xs_mask if xs_mask is not None else None,
                                                ys_mask[k] if ys_mask is not None else None,
                                                assist_states=assist_states,
                                                assist_states_m=assist_states_m)
                # write
